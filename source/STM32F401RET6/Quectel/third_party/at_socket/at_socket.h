...
typedef void (*at_evt_cb_t)(struct at_socket *socket, at_socket_evt_t event, const char *buff, size_t bfsz, rt_int64_t pkt_type);
...
struct at_socket_ops
{
    ...
    int (*at_send)(struct at_socket *socket, const char *buff, size_t bfsz, char *ip, int32_t port, enum at_socket_type type, rt_bool_t is_client);
