...
static size_t at_recvpkt_put(rt_slist_t *rlist, const char *ptr, size_t length, rt_int64_t pkt_type)
{
...
static size_t at_recvpkt_get(rt_slist_t *rlist, char *mem, size_t len, rt_int64_t *pkt_type)
{
    rt_slist_t *node = RT_NULL;
    at_recv_pkt_t pkt = RT_NULL;
    size_t content_pos = 0, page_pos = 0;
    rt_int64_t cur_pkt_type = -1;
...
static struct at_socket *alloc_socket_by_device(struct at_device *device, enum at_socket_type type)
{
    ...
    if (device->socket_ops->at_socket != RT_NULL)
    {
        idx = device->socket_ops->at_socket(device, type);
    }
    else
    {
    ...
    if (idx < 0 || idx >= device->socket_num)
    {
        goto __err;
    }
...
static void at_recv_notice_cb(struct at_socket *sock, at_socket_evt_t event, const char *buff, size_t bfsz, rt_int64_t pkt_type);
static void at_closed_notice_cb(struct at_socket *sock, at_socket_evt_t event, const char *buff, size_t bfsz, rt_int64_t pkt_type);
...
int at_bind(int socket, const struct sockaddr *name, socklen_t namelen)
{
    ...
    ip_addr_copy(local_ipaddr, device->ip_addr);
...
static void at_recv_notice_cb(struct at_socket *sock, at_socket_evt_t event, const char *buff, size_t bfsz, rt_int64_t pkt_type)
{
    ...
    at_recvpkt_put(&(sock->recvpkt_list), buff, bfsz, pkt_type);
...
int at_connect(int socket, const struct sockaddr *name, socklen_t namelen)
{
    ...
    if (sock->ops->at_connect(sock, ipstr, remote_port, sock->type, sock->is_client) < 0)
    {
        result = -1;
        goto __exit;
    }
...
int at_recvfrom(int socket, void *mem, size_t len, int flags, struct sockaddr *from, socklen_t *fromlen)
{
    ...
    if (from && sock->type == AT_SOCKET_UDP && sock->state == AT_SOCKET_OPEN)
    {
        ...
        if (!sock->is_client)
        {
            remote_port = sock->sin_port;
            strcpy(ipstr, "Local ip");
        }
        else
        {
    ...
    rt_mutex_take(sock->recv_lock, RT_WAITING_FOREVER);
    if((recv_len = at_recvpkt_get(&(sock->recvpkt_list), (char *)mem, len, &at_socket_info.data)) > 0)
    {
        rt_mutex_release(sock->recv_lock);
        goto __exit;
    }
    ...
    if (recv_len > 0)
    {
        if (at_socket_info.data != 0)
        {
            ...
            if (*fromlen >= sizeof(struct sockaddr))
            {
                ...
                sin->sin_port = at_socket_info.addr.port;
                sin->sin_addr.s_addr = at_socket_info.addr.sin_addr;
            }
        }
        result = 0;
        goto __exit;
    ...
    while (1)
    {
        ...
        {  
            ...
            rt_mutex_take(sock->recv_lock, RT_WAITING_FOREVER);
            recv_len = at_recvpkt_get(&(sock->recvpkt_list), (char *) mem, len, &at_socket_info.data);
            rt_mutex_release(sock->recv_lock);
            if (recv_len > 0)
            {
                if (at_socket_info.data != 0)
...
int at_sendto(int socket, const void *data, size_t size, int flags, const struct sockaddr *to, socklen_t tolen)
{
    ...
    case AT_SOCKET_TCP:
        ...
        if ((len = sock->ops->at_send(sock, (const char *) data, size, ipstr, remote_port, sock->type, sock->is_client)) < 0)
        {
            result = -1;
            goto __exit;
        }
    ...
    case AT_SOCKET_UDP:
        if (to)
        {
            if ((sock->state == AT_SOCKET_OPEN) || (!sock->is_client))
            {
        ...
        if ((len = sock->ops->at_send(sock, (char *) data, size, ipstr, remote_port, sock->type, sock->is_client)) < 0)
        {
            result = -1;
            goto __exit;
        }
...
struct hostent *at_gethostbyname(const char *name)
{
    ...
    if (idx < strlen(name))
    {
        if (device->socket_ops->at_domain_resolve(name, ipstr) < 0)
        {
            return RT_NULL;
        }
    }
    ...
    *res = RT_NULL;
    ...
    if (nodename != RT_NULL)
    {
        ...
        else
        {
            ...
            {
                if (device->socket_ops->at_domain_resolve((char *) nodename, ip_str) != 0)
                {
                    return EAI_FAIL;
                }
            }
 ...
 * @param backlog (ATTENTION: needs TCP_LISTEN_BACKLOG=1),This parameter is temporarily invalid
...
int at_listen(int socket, int backlog)
{
    ...
    if ((sock->type != AT_SOCKET_TCP) && (sock->type != AT_SOCKET_UDP))
    {
        LOG_E("Only TCP/UDP supports listening(%d, %d)", socket, sock->type);
        result = -1;
        goto __exit;    
    }
    ...
    if (sock->state == AT_SOCKET_CLOSED)
    {
        LOG_E("Please open socket first(%d, %d)", socket, sock->type);
        result = -1;
        goto __exit;
    }
    ...
    {
        if (sock->ops->at_connect(sock, RT_NULL, sock->sin_port, sock->type, sock->is_client) < 0)
        {
            result = -1;
            goto __exit;
        }
 ...
 */
int at_accept(int socket, const struct sockaddr *name, socklen_t *namelen)
{
    struct at_socket *sock = RT_NULL, *news_sock = RT_NULL;
    ...
    if (sock->state != AT_SOCKET_LISTEN)
    {
        LOG_E("Please listen socket first(%d, %d)", socket, sock->type);
        result = -1;
        goto __exit;
    }
    sock->state = AT_SOCKET_LISTEN;
    ...
    if (!rt_slist_isempty(&sock->recvpkt_list))
    {
        /* get receive buffer to receiver ring buffer */
        rt_mutex_take(sock->recv_lock, RT_WAITING_FOREVER);
        recv_len = at_recvpkt_get(&(sock->recvpkt_list), (char *)&incoming_info, sizeof(incoming_info), NULL);
        rt_mutex_release(sock->recv_lock);
        ...
        if (recv_len > 0)
        {
            ...
            */
            if (NULL != name) 
            {
                LWIP_ASSERT("addr valid but addrlen NULL", namelen != NULL);
                memset(&sin, 0, sizeof(sin));
                sin.sin_len = sizeof(sin);
                sin.sin_family = AF_INET;
                sin.sin_port = incoming_info.socket_addr.addr.port;
                sin.sin_addr.s_addr = incoming_info.socket_addr.addr.sin_addr;
            ...
            if (news_sock == RT_NULL)
            {
                result = -1;
                goto __exit;
            }
            news_sock->user_data = (void *)incoming_info.socket;
            news_sock->type = sock->type;
            news_sock->state = AT_SOCKET_CONNECT;
            /* set AT socket receive data callback function */
            news_sock->ops->at_set_event_cb(AT_SOCKET_EVT_RECV, at_recv_notice_cb);
            news_sock->ops->at_set_event_cb(AT_SOCKET_EVT_CLOSED, at_closed_notice_cb);
        }
    }
